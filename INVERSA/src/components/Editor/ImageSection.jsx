...
const ImageSection = ({ section, canEdit, onDelete, onUpdate, onSave, onMoveUp, onMoveDown, isFirst, isLast }) => {
  ...
  useEffect(() => {
    ...
    const handleUpdated = ({ sectionId, image_url, caption }) => {
      ...
      setCaption(caption || "");
      setLastSavedImageUrl(image_url || "");
  ...
  useEffect(() => {
    ...
    setLastSavedImageUrl(section.image_url || "");
    setLastSavedCaption(section.caption || "");
  }, [section.id]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      socket.emit("section_updated", { sectionId: section.id, image_url: imageUrl, caption });
  ...
  const buttonClass = "border rounded-lg py-2 flex items-center justify-center hover:bg-white dark:hover:bg-dark-surface disabled:opacity-40";
